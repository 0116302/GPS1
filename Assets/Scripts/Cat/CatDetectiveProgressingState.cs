...
public class CatDetectiveProgressingState : CatProgressingState {
	...
	public CatDetectiveProgressingState (Cat cat, bool canBeLured = true) : base(cat) {
		this.canBeLured = canBeLured;
	}

	public override void Update () {
		if (_target == null  && cat.currentRoom != null && !_isDisarming && !_isEnteringStaircase && !_isRetargeting) {
			// The cat does not have a target and isn't currently doing anything (probably just transitioned from another state)
